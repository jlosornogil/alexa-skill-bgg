...
const RangeListItemsIntentHandler = {
    ...
    async handle(handlerInput) {
        const speakOutput = await getGameList(handlerInput, handlerInput.requestEnvelope.request.intent.slots.start.value -1, handlerInput.requestEnvelope.request.intent.slots.end.value + 1);
        return handlerInput.responseBuilder
            .speak(speakOutput)
            //.reprompt('add a reprompt if you want to keep the session open for the user to respond')
            .getResponse();
